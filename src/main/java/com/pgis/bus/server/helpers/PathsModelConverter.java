...
public class PathsModelConverter {
	...
	public static PathModel makePathModel(List<Path_t> elems) {
		...
		input.setMoveTimeSecs(DateTimeHelper.toSeconds(first.move_time));
		input.setToRouteID(elems.get(1).direct_route_id);

		OutputWayModel output = new OutputWayModel();
		output.setDistance(last.distance);
		output.setMoveTimeSecs(DateTimeHelper.toSeconds(last.move_time));

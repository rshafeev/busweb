...
function on_selectWay(way_ind, routeParts_arr) {
	...
	for ( var i = 0; i < mainPage.getMainPageData().getWaysCount(); i++) {
		...
		if (i == way_ind) {
			...
			else {$(head).css(cssResNumbDesActiveNewBr);
...
function on_btn_calculate_click() {
	...
	$('#panel_data').html(
			"<div class='loader_text'><img src='" + loadGif + "'/></div>");
...
function on_btn_metro_click(e) {
	...
	if (g[0].src.indexOf(getContextPath()
			+ 'media/css/images/metro_selected.png') != -1)
...
function on_right_panel_show() {
	...
	if (getMainPage().rightPanelVisible == false) {
		...
		document.img.src = getContextPath()
				+ 'media/css/images/arrow_right.png';
